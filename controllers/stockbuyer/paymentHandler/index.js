...
exports.pay= async (req,res,next)=>{
   ...
   try {
        ...
        const session = await stripe.checkout.sessions.create({
          ...
          line_items: [
            {
              price_data: {
                ...
                product_data: {
                  ...
                  // images: [req.body.images[0].image],
                },
          ...
          success_url: 'http://localhost:3000/stockbuyer/dash/orders',
          cancel_url:  'http://localhost:3000/stockbuyer/dash/orders'
        });
...
exports.payRemainingAmount = async (req,res,next)=>{
  ...
  conn.query(sql, [req.body.id],async (err,data1)=>{
    ...
    if (data1[0]?.code==req.body.code) {
        if ((req.body.unit_price*req.body.quantity*100 - req.body.advance*100)>0) {
      ...
      res.send({
        statusCode:110,
        message:"Entered amount not allowed"
      });
    }
    }else{
      res.send({
          ...
          try {
            ...
            conn.beginTransaction((err)=>{
                ...
                conn.query(sql2, [data.amount_total/100,farmerid,buyerid,reqid,data.payment_intent],(err,data2)=>{
                  console.log(data.amount_total/100,farmerid,buyerid,reqid,data.payment_intent)
                    if (err) { 
                        conn.rollback(()=> {
                            console.log(err.message)
                            return next(new AppError(err,500))
